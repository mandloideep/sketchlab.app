...
export async function mountEditor(
  ...
  const layersPanel = new LayersPanel(editor, {
    onCollapseChange: (collapsed) => {
      layersBtn?.setAttribute("aria-expanded", String(!collapsed));
      if (!collapsed) settingsPanel.collapse(); // the two right-docked trays share space
    },
  });
  const settingsPanel = new SettingsPanel(editor, {
    onCollapseChange: (collapsed) => {
      settingsBtn?.setAttribute("aria-expanded", String(!collapsed));
      if (!collapsed) layersPanel.collapse();
    },
  });
  unsubs.push(
  ...
  settingsBtn = h("button", {
    class: "btn btn--icon",
    title: "Show / hide View controls",
    "aria-label": "Toggle view controls panel",
    "aria-expanded": "false",
    html: ICON_SETTINGS,
